...
function onMessage(userId, msg) {
    ...
    if (isMessageForCurrentDialog(userId, msg.dialog_id)) {
        ...
        showMessage(userId, msg, messageAttachmentFileId, null);
    }
                ...
                messages.items.forEach(function(item, i, arr) {
                    ...
                    if (item.hasOwnProperty("attachments")) {
                        if (item.attachments.length > 0) {
                            ...
                            fileType = item.attachments[0].type;
                        }
                    }

                    var messageHtml = buildMessageHTML(messageText, messageSenderLogin, messageDateSent, messageAttachmentFileId, messageId, fileType);
...
function clickSendMessage() {
    ...
    sendMessage(currentText, null, null);
}
...
function clickSendAttachments(inputFile) {
    ...
    QB.content.createAndUpload({
        ...
        if (err) {
            ...
            sendMessage("[attachment]", uploadedFile.id, inputFile.type);
...
function sendMessage(text, attachmentFileId, fileType) {
    ...
    if (attachmentFileId !== null) {
        if (fileType.indexOf("video") !== -1) {
            msg['extension']['attachments'] = [{ id: attachmentFileId, type: 'video' }];
            typeUpload = "video";
        } else {
    ...
    if (currentDialog.type === 3) {
        ...
        if (attachmentFileId === null) {
            showMessage(currentUser.id, msg, null, typeUpload);
        } else {
            showMessage(currentUser.id, msg, attachmentFileId, typeUpload);
        }
    } else {
...
// show messages in UI
function showMessage(userId, msg, attachmentFileId, typeFile) {
    
    var userLogin = getUserLoginById(userId);
    var messageHtml = buildMessageHTML(msg.body, userLogin, new Date(), attachmentFileId, msg.id, typeFile);
